...
class RpcApi(private val client: RpcClient) {
    ...
    @Throws(RpcException::class)
    fun sendTransaction(transaction: Transaction, signer: Account, recentBlockHash: String? = null): String {
        return sendTransaction(transaction, listOf(signer), recentBlockHash)
    }

    @Throws(RpcException::class)
    fun sendTransaction(transaction: Transaction, signers: List<Account>, recentBlockHash: String?): String {
    ...
    @Throws(RpcException::class)
    fun getBalance(account: PublicKey, commitment: Commitment? = null): Long {
