 ...
 * @创建时间： 2021/12/5
 */
public class WalletManageAdapter extends BaseQuickAdapter<WalletETH, BaseViewHolder> {
    private final boolean isNeedDelete;

    public WalletManageAdapter(List<WalletETH> datas, boolean isNeedDelete) {
        super(R.layout.activity_wallet_manage_item, datas);
        this.isNeedDelete = isNeedDelete;
    }

    @Override
    protected void convert(@NotNull BaseViewHolder baseViewHolder, WalletETH walletManageBean) {
        if (isNeedDelete) {
