...
public class WalletTron {
    ...
    public boolean open(String password) {
        if (encPrivateKey != null) {
            setPrivateKey(WalletTronManager.decryptPrivateKey(encPrivateKey, password));
            return isOpen();
        } else {
            return false;
        }
    ...
    public String getAddress() {
        if (mECKey != null) {
            return WalletTronManager.encode58Check(mECKey.getAddress());
        } else if (publicKey != null) {
            return WalletTronManager.encode58Check(ECKey.fromPublicOnly(publicKey).getAddress());
        } else {
            return address;
        }
    }
