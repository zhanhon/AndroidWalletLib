...
public class WalleETHManager {
     ...
     * 区块链服务器地址
     */
    public static String URL = "http://13.229.173.84:8545";
    ...
    public static boolean isETHValidAddress(String input) {
        ...
        return isValidAddress(input, 40);
    }
     ...
     * @return
     */
    public static boolean isValidAddress(String input, int length) {
        ...
        return cleanInput.length() == length;
    }
...
//            BigInteger GAS_LIMIT = Convert.toWei(BigDecimal.valueOf(60000), Convert.Unit.GWEI).toBigInteger();
            ...
            // nonce +1 重试
            log(String.format("sendTransaction: hash = %s, nonce = %s", transactionHash, nonce));
            if (transactionHash == null) {
                if (resetCount < maxResetCount) {
                    nonce = nonce.add(BigInteger.valueOf(1));
                    resetCount += 1;
                    log(String.format("sendTransaction reset: count = %s, nonce = %s", resetCount, nonce));
