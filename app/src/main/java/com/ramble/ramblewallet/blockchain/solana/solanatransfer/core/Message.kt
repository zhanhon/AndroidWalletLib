...
import com.ramble.ramblewallet.blockchain.solana.solanatransfer.utils.ShortVectorEncoding
...
class Message {
    ...
    private var feePayer: TransactionAccount? = null
    ...
    fun serialize(): ByteArray {
        ...
        for (instruction in instructions) {
            ...
            for (i in 0 until keysSize) {
                keyIndices[i] =
                    findAccountIndex(keysList, instruction.keys.get(i).publicKey).toByte()
            }
        ...
        val bufferSize =
            (MessageHeader.HEADER_LENGTH + RECENT_BLOCK_HASH_LENGTH + accountAddressesLength.size
                    + accountKeysSize * TransactionPublicKey.PUBLIC_KEY_LENGTH + instructionsLength.size
                    + compiledInstructionsLength)

        val out = ByteBuffer.allocate(bufferSize)
        val accountKeysBuff =
            ByteBuffer.allocate(accountKeysSize * TransactionPublicKey.PUBLIC_KEY_LENGTH)
            ...
            if (accountMeta.signer) {
                ...
                if (!accountMeta.writable) {
                    messageHeader!!.numReadonlySignedAccounts =
                        messageHeader!!.numReadonlySignedAccounts.inc()
                }
            } else {
                if (!accountMeta.writable) {
                    messageHeader!!.numReadonlyUnsignedAccounts =
                        messageHeader!!.numReadonlyUnsignedAccounts.inc()
                }
            }
        }
    ...
    fun setFeePayer(feePayer: TransactionAccount?) {
    ...
    private fun findAccountIndex(
        accountMetaList: List<AccountMeta>,
