...
class Transaction {
    ...
    fun sign(signer: TransactionAccount) {
        sign(listOf(signer))
    }

    fun sign(signers: List<TransactionAccount>) {
    ...
    fun serialize(): ByteArray {
        ...
        val out: ByteBuffer =
            ByteBuffer.allocate(signaturesLength.size + signaturesSize * SIGNATURE_LENGTH + serializedMessage!!.size)
    ...
    class Builder {
        ...
        fun addInstruction(transactionInstruction: TransactionInstruction) =
            apply { transaction.addInstruction(transactionInstruction) }

        fun setRecentBlockHash(recentBlockHash: String) =
            apply { transaction.setRecentBlockHash(recentBlockHash) }

        fun setSigners(signers: List<TransactionAccount>) = apply { transaction.sign(signers) }
